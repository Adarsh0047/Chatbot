...
print(hrefs)
with open(f"{job_name} {place}_links.txt", "w") as f:
    for href in hrefs:
        f.write(href)
        f.write("\n")
def extract_data(url, id, title):
    ...
    try:
        ...
        with open(f"{job_name} {place}.txt", "a", encoding="utf-8") as f:
           f.writelines("###ID")
           f.writelines(id)
           f.writelines("\n")
           f.writelines("###TITLE")
        ...
        pass
with open(f"{job_name} {place}_links.txt", "r") as f:
    links = f.readlines()
print(links[0][:-2])
